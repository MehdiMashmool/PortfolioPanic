...
const SparklineChart: React.FC<SparklineChartProps> = ({
  ...
  const startValue =
    referenceValue !== undefined ? referenceValue : data[0].value;
  ...
  return (
    <div ref={containerRef} className={cn("h-[30px] w-full", className)}>
      <LineChart
        width={width}
        data={formattedData}
      ...
      >
        ...
        />
        <YAxis domain={[enhancedMin, enhancedMax]} hide={true} />
        {showTooltip && (
          <Tooltip
        ...
        {areaFill && (
          <>
            <defs>
              <linearGradient
                id={`sparklineGradient-${assetType || "default"}`}
                x1="0"
                y1="0"
                x2="0"
                y2="1"
        ...
        <Line
          ...
          activeDot={
            showTooltip
              ? { r: 4, stroke: lineColor, strokeWidth: 2, fill: "#1A1F2C" }
              : false
          }
        />
      </LineChart>
