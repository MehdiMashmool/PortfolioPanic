...
const PortfolioValue = ({
  ...
  return (
    <div
      className={`relative ${
        compactMode ? "p-3" : "p-4"
      } bg-panel/50 rounded-lg border border-panel-light/50`}
    >
      <div
        className={`${
          compactMode ? "text-xl" : "text-2xl"
        } text-gray-400 font-medium flex items-center mb-1`}
      >
        Portfolio Value
        <TooltipProvider>
          <Tooltip>
            ...
            >
              Your total portfolio value includes cash and all investments at
      ...
      <div
        className={`${compactMode ? "text-xs" : "text-sm"} flex items-center ${
          netWorthChange >= 0 ? "text-profit" : "text-loss"
        }`}
      >
        {netWorthChange >= 0 ? (
        ...
        <span
          className={`${
            netWorthChange >= 0
              ? "text-profit drop-shadow-[0_0_5px_rgba(16,185,129,0.4)]"
              : "text-loss drop-shadow-[0_0_5px_rgba(239,68,68,0.4)]"
          }`}
        >
          {netWorthChange >= 0 ? "+" : ""}
